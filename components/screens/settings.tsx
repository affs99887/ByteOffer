...
export function SettingsScreen() {
  ...
  function saveProfile() {
    ...
    startSave(async () => {
      ...
      if (res.ok) setSavedName(res.data.name ?? next);
      setEditing(false);
    });
  }
    ...
    <div data-screen-label="设置" className="bo-enter" style={{ maxWidth: '860px', margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '22px 24px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '18px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '12px', color: 'var(--ink3)', marginTop: '4px' }}>{tier ? `${tier.toUpperCase()} · ` : "LV.24 · 黄金题手 · "}{email}</div>
        </div>
        {editing ? (
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '8px 24px', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 0', borderTop: '1px solid var(--divider)', cursor: 'pointer' }}><span style={{ fontSize: '14px', color: 'var(--ink)' }}>绑定邮箱</span><span style={{ display: 'flex', alignItems: 'center', gap: '8px', fontFamily: "'JetBrains Mono',monospace", fontSize: '12.5px', color: 'var(--ink3)' }}>{email}<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 6l6 6-6 6"></path></svg></span></div>
